...
router.get('/', ...GamesValidation.allGame, validatePayload, GamesController.getAllMyGames); // Получить информацию о всех своих играх (с филльтрацией)
router.post('/', ...GamesValidation.createGame, validatePayload, GamesController.createGame); // Создать игру

router.get('/:gameId', validatePayload, GamesController.infoAboutGame); // Получить информацию об игре
...
router.post('/:gameId/hidden', validatePayload, GamesController.hidden); // Загадать число
