...
export class UsersValidation {
  ...
  static leaderboard = [
    query('sort[field]')
      ...
      .withMessage('Value is not includes enum STATS'),
    query('sort[type]')
      .optional()
      .custom((value) => Object.values(SORT_DIRECTION).includes(value))
      .withMessage('Value is not includes enum SORT_DIRECTION'),
    query('from').optional().isDate().withMessage("Value is not Date in format: 'YYYY/MM/DD' or 'YYYY-MM-DD'"),
    query('to').optional().isDate().withMessage("Value is not Date in format: 'YYYY/MM/DD' or 'YYYY-MM-DD'"),
    query('offset')
      .optional()
      .isInt({ allow_leading_zeroes: false })
      .withMessage('Value is not number')
